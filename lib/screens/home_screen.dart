...
import 'package:nova/services/http_service.dart';
import 'package:nova/widgets/fsLogiTechOverlay.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  OverlayEntry? homeOverlay;
  GlobalKey bellKey = GlobalKey();
  bool isOverlayOpen = false;
  late IO.Socket socket;
  late UserData driverDetail;
  ...
  void initState() {
    ...
    driverDetail = HiveService().getDriverDetail();
    socket.connect();
    socket.onConnect((_) {
      socket.emit('onConnection', {"userId": driverDetail.employeeId});
    });

    socket.on('riderNotification', (data) async {
      CalendarResponseModel calendarResponseModel =
          await novaBloc.outCalendarResponse.first;

      calendarResponseModel.todayOrders =
          TodayOrders.fromJson(data['todayOrders']);

      calendarResponseModel.monthOrders = List<MonthOrder>.from(data['monthOrders']
          .map((x) => MonthOrder.fromJson(x))
      );

      calendarResponseModel.notifications = List<InAppNotification>.from(
        data['notifications']
            .map((notification) => InAppNotification.fromJson(notification))
      );

      calendarResponseModel.unreadCount = data['unReadCount'];

      novaBloc.inCalendarResponse.add(calendarResponseModel);
    });
          ...
          builder: (context, outCalendarResponseSnapshot) {
            return Scaffold(
              appBar: AppBar(
                ...
                actions: [
                  outCalendarResponseSnapshot.data != null
                      ? InkWell(
                          onTap: () async {
                            showDialog(
                                context: context,
                          ...
                          child: SvgPicture.asset(
                            "assets/icons/calendar-blank.svg",
                            height: 25,
                            width: 25,
                          ),
                        )
                  ...
                  Padding(
                    ...
                    child: InkWell(
                      onTap: () async{
                        if (outCalendarResponseSnapshot.hasData && outCalendarResponseSnapshot.data!.unreadCount != 0) {
                          novaBloc.add(ReadInAppNotifications(userId: driverDetail.employeeId!, previousCalendarResponseModel: outCalendarResponseSnapshot.data!));
                        }

                        RenderBox box = bellKey.currentContext!
                            .findRenderObject() as RenderBox;
                        Offset position = box.localToGlobal(Offset.zero);
                        homeOverlay = FsLogitechOverlay().notificationOverlay(
                            position,
                            box.size,
                            outCalendarResponseSnapshot, () {
                          setState(() {
                          ...
                          Navigator.of(context).push(
                            MaterialPageRoute(
                              builder: (context) => DeliveriesListScreen(
                                deliveriesList: outCalendarResponseSnapshot
                          ...
                          orderPool.addAll(outCalendarResponseSnapshot.data!.todayOrders!.pending!);
                          orderPool.addAll(outCalendarResponseSnapshot.data!.todayOrders!.ongoing!);
                          orderPool.addAll(outCalendarResponseSnapshot.data!.todayOrders!.completed!);
                          orderPool.addAll(outCalendarResponseSnapshot.data!.todayOrders!.failed!);
                          try {
                            Order orderClicked = orderPool
                                .firstWhere((element) => element.orderId == id);
                            Navigator.of(context).push(
                              MaterialPageRoute(
                                builder: ((context) =>
                                    DeliveryDetailScreen(order: orderClicked)),
                              ),
                            );
                        ...
                        Overlay.of(context)!.insert(homeOverlay!);
                        setState(() {
                          isOverlayOpen = true;
                        });
                      },
                      child: Stack(children: [
                        ...
                        outCalendarResponseSnapshot.hasData ? outCalendarResponseSnapshot.data!.unreadCount != 0 
                            ? Align(
                                alignment: Alignment.center,
                                child: Padding(
              ...
              body: SmartRefresher(
                ...
                onRefresh: () async {
                  novaBloc.add(
                      GetCalendarEvent(refreshController: refreshController));
                },
                                                                ...
                                                                child: Row(
                                                                  children: [
                                                                    ...
                                                                    Padding(
                                                                      ...
                                                                      child:
                                                                          Text(
                                                                        AppConfig().timeFormat.format(outTodaysStatusResponseSnapshot
                                                                            .data!
                                                                            .todayOrders!
                                                                  ...
                                                                  InkWell(
                                                                    onTap: () {
                                                                      Navigator.of(
                                                                              context)
                                                                          .push(
                                                                              MaterialPageRoute(builder: ((context) => DeliveryDetailScreen(order: outTodaysStatusResponseSnapshot.data!.todayOrders!.ongoing![ongoingIndex]))));
                                                                    },
                                                                    child: Row(
                                                                      children: const [
                                                                        Text(
                                                                  ...
                                                                  Padding(
                                                                    ...
                                                                    child:
                                                                      ...
                                                                      onTap:
                                                                          () async {
                                                                        await launchUrl(
                                                                            Uri.parse("$mapUrl&query=${outTodaysStatusResponseSnapshot.data!.todayOrders!.ongoing![ongoingIndex].delivery!.location!.latitude},${outTodaysStatusResponseSnapshot.data!.todayOrders!.ongoing![ongoingIndex].delivery!.location!.longitude}"));
                                                                      },
                                                                      child:
                                                                          Row(
                                                                        children: const [
